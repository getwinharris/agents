...
export class CloudflarePlugin implements BuildPlugin {
	...
	async generateEntryPoint(ctx: BuildContext): Promise<string> {
		...
		if (ctx.dbEntry) {
			throw new Error(
				`[bapX] Custom persistence (db.ts) is not supported on the Cloudflare target. ` +
...
const ${workflowClassName(workflow.name)} = class ${workflowClassName(workflow.name)} extends workflowExtension${index}.base(Agent) {
  ...
  async onFiberRecovered(ctx) {
    if (ctx.name?.startsWith('bapX:workflow:')) {
      return handleBapxWorkflowFiberRecovered(ctx, this, ${JSON.stringify(workflow.name)});
    }
...
export { Wrapped${workflowClassName(workflow.name)} as ${workflowClassName(workflow.name)} };`,
		...
		return `
// Auto-generated by bapX (target: cloudflare)
...
} from '@bapX/runtime/cloudflare/internal';
import { registerApiProvider, registerProvider } from '@bapX/runtime';
...
for (const name of Object.keys(userCloudflare)) {
  ...
  if (reservedCloudflareExportNames.has(name)) {
    throw new Error('[bapX] cloudflare.ts export "' + name + '" conflicts with a Bapx-generated Worker export. Rename the authored export.');
  }
}
...
if ('fetch' in cloudflareHandlers) {
  throw new Error('[bapX] cloudflare.ts default export must not define fetch. Use app.ts for custom HTTP handling.');
}
...
const INTERNAL_WORKFLOW_INVOKE_PATH = '/_bapX/internal/workflow-invoke';
const dispatchQueue = {
  async enqueue(input) {
    const identity = agentIdentities[input.agent];
    const binding = env?.[identity?.bindingName];
    if (!binding) throw new Error('[bapX] dispatch() target agent "' + input.agent + '" Durable Object binding is unavailable.');
    const response = await fetchAgent(binding, input.id, new Request('https://bapX.invalid' + INTERNAL_DISPATCH_PATH, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
    }));
    if (!response.ok) throw new Error('[bapX] dispatch() target agent "' + input.agent + '" rejected durable admission with status ' + response.status + '.');
...
function createRunStoreForRequest(doInstance) {
  // Composite run store: per-workflow-DO records plus the BapxRegistry
  // index DO for cross-deployment lookup/listing.
  const sql = doInstance?.ctx?.storage?.sql;
  if (!sql) {
    throw new Error('[bapX] Durable Object SQLite storage is unavailable — cannot create the run store. Bapx Durable Object classes require SQLite-backed storage.');
  }
...
function createEventStreamStoreForInstance(doInstance) {
  ...
  const sql = doInstance?.ctx?.storage?.sql;
  if (!sql) {
    throw new Error('[bapX] Durable Object SQLite storage is unavailable — cannot create the event stream store. Bapx Durable Object classes require SQLite-backed storage.');
  }
...
function assertAgentsDurabilityApi(doInstance, method) {
  if (typeof doInstance[method] !== 'function') {
		throw new Error(
			'[bapX] The installed "agents" package does not provide the required Cloudflare Agents SDK method "' +
				method +
...
async function handleBapxWorkflowFiberRecovered(ctx, doInstance, workflowName) {
  if (!ctx.name || ctx.name !== 'bapX:workflow:' + doInstance.name) return;
  const interruptedRunId = doInstance.name;
  const runStore = createRunStoreForRequest(doInstance);
  await failRecoveredRun({
    workflowName,
    runId: interruptedRunId,
    request: new Request('https://bapX.invalid/workflows/' + encodeURIComponent(workflowName), { method: 'POST' }),
    error: new Error('Bapx workflow execution was interrupted. Start a new workflow run explicitly if retry is appropriate.'),
    runStore,
    ...
    return runWithInstanceContext(doInstance, identity, async () => {
      const receipt = await admitDetachedWorkflow({
        ...
        startWorkflowAdmission: (runId, run) => {
          ...
          const completion = doInstance.runFiber('bapX:workflow:' + runId, () => {
            admission.resolve();
...
configureBapxRuntime({
  ...
  admitWorkflow: async ({ workflowName, input }) => {
    ...
    const binding = env?.[identity?.bindingName];
    if (!binding) throw new Error('[bapX] invoke() target workflow Durable Object binding is unavailable.');
    const runId = generateWorkflowRunId();
    const response = await fetchAgent(binding, runId, new Request('https://bapX.invalid' + INTERNAL_WORKFLOW_INVOKE_PATH, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ input }),
    }));
    if (!response.ok) throw new Error('[bapX] invoke() target workflow rejected durable admission with status ' + response.status + '.');
    return response.json();
  },
  channelHandlers,
...
// through. The default bapX() handler is available for them to mount
// however they want; this file does not impose a composition.
const bapXApp = userApp;
if (!bapXApp || typeof bapXApp.fetch !== 'function') {
  throw new Error(
    '[bapX] app.ts default export must be a Hono app or an object with a fetch(request, env, ctx) method.'
  );
}`
...
// they author their own app.ts). The default mounts \`bapX()\` at root
// and renders canonical Bapx envelopes for unmatched paths.
const bapXApp = createDefaultBapxApp();`
}

export default {
  ...cloudflareHandlers,
  fetch(request, env, ctx) {
    return bapXApp.fetch(request, env, ctx);
  },
};
`;
	}
	...
	async viteInputs(ctx: BuildContext): Promise<ViteCloudflareInputs> {
		const bapXBindings: Array<{ name: string; class_name: string }> = ctx.agents.map((agent) => ({
		...
		const FLUE_REGISTRY_BINDING = { name: 'FLUE_REGISTRY', class_name: 'BapxRegistry' };
		bapXBindings.push(FLUE_REGISTRY_BINDING);
		...
		// Bapx's contributions to the wrangler config. Everything else in the
		...
		const merged = mergeBapxAdditions(userConfig, additions);
		...
		// Bapx no longer emits a Dockerfile. Users who use container sandboxes
...
function validateCloudflareExportNames(ctx: BuildContext): void {
	const entries = [
		...
		{ name: 'BapxRegistry', source: 'Bapx registry' },
	];
	...
	if (!conflicts) return;
	throw new Error(
		`[bapX] Cloudflare target generated conflicting Worker export name(s): ${conflicts}. Rename the conflicting agent or workflow file.`,
	);
}
...
function validateCloudflareAgentNames(ctx: BuildContext): void {
	...
	throw new Error(
		`[bapX] Cloudflare target requires agent and workflow filenames to use lower-kebab-case so ` +
...
function agentClassName(name: string): string {
	return `Bapx${pascalCaseName(name)}Agent`;
}

function workflowClassName(name: string): string {
	return `Bapx${pascalCaseName(name)}Workflow`;
}
