...
const lifecycle = await startBapxNodeServer({
  port: parseInt(process.env.PORT || '3000', 10),
});
async function stop(exitCode) {
  setTimeout(() => {
    console.error('[bapX] Shutdown timed out, exiting.');
...
`;
	...
	generateRuntimeEntryPoint(ctx: BuildContext, options: { deployment?: boolean } = {}): string {
		...
		// Hono that mounts `bapX()` and renders canonical error envelopes.
		const userAppImport = '';
		...
		return `
// Auto-generated by bapX (target: node)
...
export async function loadBapxNodeApplication(options = {}) {
...
if (!userPersistenceAdapter || typeof userPersistenceAdapter.connect !== 'function') {
  throw new Error('[bapX] db.ts must default-export a PersistenceAdapter with a connect() method.');
}
let executionStore;
let runStore;
...
try {
  ...
  throw new Error('[bapX] Failed to initialize persistence from db.ts: ' + (error instanceof Error ? error.message : error), { cause: error });
}`
		: `// Default persistence for Node — in-memory SQLite, process lifetime.
// Under local \`bapX dev\`, FLUE_DEV_SQLITE_PATH points this at a disk file so
...
// Seed the public bapX() sub-app before this loaded application is installed
// into its listener. User app.ts files that call bapX() at module evaluation
...
configureBapxRuntime({
  ...
  admitWorkflow: ({ workflowName, input }) => {
    ...
    if (!workflow) throw new Error('[bapX] Internal workflow admission target is not registered.');
    return admitDetachedWorkflow({
      workflowName,
      workflow,
      input,
      request: new Request('https://bapX.invalid/_internal/workflows/' + encodeURIComponent(workflowName), { method: 'POST' }),
...
try {
  ...
  runInRuntime(() => console.error('[bapX] Startup submission reconciliation failed:', error));
}
...
// listener through. bapX() is available for them to mount, but the
// composition is theirs to author.
const bapXApp = userApp;
if (!bapXApp || typeof bapXApp.fetch !== 'function') {
  throw new Error(
    '[bapX] app.ts default export must be a Hono app or an object with a fetch(request) method.'
  );
}`
...
const bapXApp = createDefaultBapxApp();`
  ...
  let disposing;
  return {
    fetch: (request, env) => runInRuntime(() => bapXApp.fetch(request, env)),
    enterActivity() {
    ...
    async stop(timeoutMs = 30_000) {
      ...
      disposing = runInRuntime(async () => {
        ...
        if (errors.length > 1) throw new AggregateError(errors, '[bapX] Node application shutdown failed.');
      });
      return disposing;
    },
    closeSync() {
    ...
    devLifecycle?.dispose();
    restoreOutput();
    if (cleanupErrors.length) throw new AggregateError([error, ...cleanupErrors], '[bapX] Node application startup failed.');
    throw error;
  }
}

export async function startBapxNodeServer(options = {}) {
  const application = await loadBapxNodeApplication(options);
  ...
  const server = serve({
    ...
    if (!options.quiet) console.log('[bapX] Server listening on http://localhost:' + port);
    options.onReady?.();
    resolveReady();
  });
  ...
  return {
    async stop() {
      ...
      stopping = (async () => {
        ...
        await httpClosed;
        if (errors.length === 1) throw errors[0];
        if (errors.length > 1) throw new AggregateError(errors, '[bapX] Node server shutdown failed.');
      })();
      return stopping;
    },
    closeSync() {
