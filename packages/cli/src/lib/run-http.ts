...
export function parseHeaders(values: readonly string[]): Record<string, string> {
	...
	for (const value of values) {
		...
		if (separator <= 0) throw new TypeError(`[bapX] Invalid header: ${JSON.stringify(value)}.`);
		const name = value.slice(0, separator).trim();
		const headerValue = value.slice(separator + 1).trim();
		if (!name || !isHeaderName(name)) {
			throw new TypeError(`[bapX] Invalid header name: ${JSON.stringify(name)}.`);
		}
		if (/[^\t\x20-\x7e\x80-\xff]/.test(headerValue)) {
			throw new TypeError(`[bapX] Invalid value for header ${JSON.stringify(name)}.`);
		}
...
function joinMount(origin: URL, mount: string): string {
	if (!mount.startsWith('/')) {
		throw new TypeError(`[bapX] Authored mount path must start with "/": ${JSON.stringify(mount)}.`);
	}
...
function absoluteHttpUrl(value: string, label: string): URL {
	...
	try {
		...
		throw new TypeError(`[bapX] ${label} must be an absolute URL: ${JSON.stringify(value)}.`);
	}
	if (url.protocol !== 'http:' && url.protocol !== 'https:') {
		throw new TypeError(`[bapX] ${label} must use http or https: ${JSON.stringify(value)}.`);
	}
	return url;
}
