...
Place this export in `channels/stripe.ts`. Bapx discovers it and serves
`POST /channels/stripe/webhook` relative to the `bapX()` mount.
...
Stripe's event unions follow the installed SDK version. Bapx forwards verified
