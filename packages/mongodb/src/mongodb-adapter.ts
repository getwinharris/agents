...
export function mongodb(runner: MongoRunner, options: MongoOptions = {}): PersistenceAdapter {
	...
	return {
		async migrate() {
			...
			if (topology.kind === 'standalone' || !topology.transactions) throw new TypeError('@flue/mongodb requires a replica set, Atlas, or a transaction-capable sharded cluster.');
			...
			while (true) {
				...
				const lock = await meta.findOneAndUpdate(
					{ _id: 'migration_lock', $or: [{ ownerId }, { leaseExpiresAt: { $lt: now } }, { leaseExpiresAt: { $exists: false } }] },
			...
			const heartbeat = setInterval(() => {
				renewal = renewal.then(async () => {
					const result = await meta.updateOne({ _id: 'migration_lock', ownerId }, { $set: { leaseExpiresAt: Date.now() + MIGRATION_LEASE_MS } }).catch(() => null);
			...
			try {
				...
				await renewal;
				if (lockLost || !await meta.findOne({ _id: 'migration_lock', ownerId })) throw new TypeError('MongoDB migration lock ownership was lost.');
		...
		connect() {
			if (!migrated) throw new TypeError('@flue/mongodb connect() requires a successful migrate() first.');
			return { executionStore: { sessions: new MongoSessionStore(runner, prefix), submissions: new MongoSubmissionStore(runner, prefix) }, runStore: new MongoRunStore(runner, prefix), eventStreamStore: new MongoEventStreamStore(runner, prefix) };
		},
		async close() { if (!closed) { closed = true; await runner.close(); } },
	};
}
