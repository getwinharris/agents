...
| E2B | Default hosted remote sandbox path | `e2b` SDK-backed lifecycle through bapX-owned sandbox commands and browser profile allocation | Adapter and docs exist; hosted lifecycle command surface is tracked in issue #87 |
| Vercel | Vercel Sandbox catalog | `vercel` for authorized project linking, environment, build, deploy, and inspection operations | Sandbox exists; deployment connector and CLI capability is planned |
...
- AgentBrowser is the preferred `bapX browse` backend and supplies task-space isolation plus a Playwright-style JavaScript facade through `ego-browser`. The MIT-licensed `bapXai/AgentBrowser` fork now includes an open Linux Chrome/Chromium host. The separately distributed macOS app remains outside that source boundary.
- The bapX wrapper gives each namespace/session a distinct owner-only Linux profile under `.agents/browser/profiles/`. The Linux host is verified for one-process VPS automation; do not claim cross-process task-space persistence, macOS profile migration, kernel-customized snapshot parity, or GUI handoff.
- Vercel `agent-browser` remains the explicitly selected `--engine vercel` fallback. It must not be described as the preferred backend.
- Playwright provides deterministic cross-browser regression tests, locators, traces, screenshots, downloads, and repeatable UI validation using a bapX-owned isolated profile.
- Hosted browser profiles are server-derived from the authorized account, business, project, and shared project-browser scope. The human user and that user's project agents may share the same project profile. Another user or project must receive a different profile root and must never reuse a desktop/personally logged-in Chrome profile.
