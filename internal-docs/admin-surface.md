...
- `GET /api/projects` lists directories below `/root/bapx.in/projects/` and reads only the private `.bapx-project.json` metadata written by the import owner. Completed imports preserve their operation ID, completion status, and verified commit in this listing so Admin can render durable evidence after a reload rather than relying only on transient submission text.
...
- The browser request contract is `{ "repositoryUrl": { "repositoryUrl": "<supported GitHub URL>", "projectSlug": "<confirmed slug>", "confirmed": true } }`. All three values are required. The server rejects URL-only, missing-slug, missing-confirmation, and false-confirmation requests before directory creation or Git execution, so direct same-origin requests cannot bypass the operator confirmation shown in the Admin UI.
