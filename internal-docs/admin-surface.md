...
- The provider verifies that GitHub returned every requested capability before caching or returning a token. Callers and the metadata resolver do not cache or refresh credentials independently.
- The outbound token request is bounded by a ten-second abort signal. Configuration, signing, invalid permission requests, transport, non-success responses, malformed JSON, under-scoped responses, and malformed token payloads all map to the same secret-free `github_installation_unavailable` error.
...
- Changing these environment values or this module requires rebuilding and restarting the existing `apps-www` service. Validate metadata-only and Contents-read request bodies, cross-scope cache isolation, per-scope cache reuse and near-expiry refresh, malformed configuration and unsupported-scope rejection, request cancellation, under-scoped response rejection, and secret-free failures before deployment.
