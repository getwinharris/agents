...
description: Set up a bapX project automatically or create your first agent manually.
lastReviewedAt: 2026-05-29
---

import CopyPrompt from '../../../components/docs/CopyPrompt.astro';

**bapX** is a TypeScript framework for building AI agents. Define your agents using the exact same harness-driven architecture used by Claude Code and other coding agents to build truly autonomous software. Write your agents with bapX and then run them anywhere (local CI, Node.js server, Cloudflare, etc.).
...
- **A coding agent (recommended)** — Several bapX features assume you have a coding agent available to run locally (Claude Code, Codex, etc.).
- **A container sandbox (optional)** — bapX includes a built-in virtual sandbox, suitable for many agentic workloads. If you need a real VM, consider a [container sandbox](/docs/ecosystem/#sandboxes).
...
In a new directory, install bapX and initialize your target. The `bapX init` command creates `bapX.config.ts`; you will add an agent module in the next step.

```bash
npm install @bapX/runtime
npm install --save-dev @bapX/cli
echo 'ANTHROPIC_API_KEY="your-api-key"' > .env
npx bapX init --target node # or: --target cloudflare
```
...
The default export of `agents/hello-world.ts` is what registers the agent — the filename makes it available in bapX as `hello-world`.
...
```bash
npx bapX run hello-world --input '{"message":"Tell me a joke."}'
```
...
Congratulations! You have created and run your first bapX agent. From here, you can shape its behavior, add capabilities, or deploy it as part of an application.
