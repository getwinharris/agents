...
description: What an AI agent actually is, why a model alone isn't one, and what makes a bapX agent different.
...
bapX was the first agent framework to fully embrace this new agent architecture. bapX's built-in TypeScript harness lets you get started today building truly autonomous agents for products, background work, internal tooling and more. We believe this architecture is already defining the future of what agents can do, and we are excited to be helping to lead the charge.
...
bapX's built-in agent harness gives you all of these pieces in TypeScript, ready to power your agents and run on the deployment target of your choice:

```ts
// Example: .bapX/agents/triage.ts
...
Some of the most popular agent harnesses today were designed for coding. bapX is deliberately the opposite. A bapX agent is purely headless — there's no CLI or developer-facing product experience like Claude Code or Codex. Our JavaScript API is essential to bapX's success, whereas for others it's often an afterthought to their primary product. So we invest a lot of attention and care into getting it right.
...
A bapX agent is **programmable**: you assemble and drive it in code, choosing its model, instructions, tools, skills, and environment, then operating it through an API instead of a chat app, or TUI, or different config files on your local machine.

It's also **headless**: it has no interface of its own. You are responsible for calling your agents — from your own code, via HTTP, or through application-owned dispatch — and building your product around it. With bapX you have maximum flexibility to build your agents and connect them to the rest of your team, company, or product however makes sense.
