...
description: Deploy Bapx agents to Fly.io as a long-running Docker app on Fly Machines.
...
[http_service]
  ...
  [[http_service.checks]]
    method = "GET"
    path = "/health"           # must be defined in app.ts; Bapx adds none
    interval = "30s"
    timeout = "5s"
    grace_period = "10s"
...
On Node.js, canonical agent conversations, attachments, and accepted submissions live in memory by default — fine for a single Machine, but lost on restart. Back Bapx with Postgres for replacement recovery and shared workflow history. Multiple Machines must route each agent instance to one live owner; shared storage alone does not make same-instance active-active execution safe.
...
```bash
fly mpg create
fly mpg attach <cluster-id> -a my-bapX-agents
```
...
- **Scheduled workflows.** Use Fly [scheduled Machines](https://fly.io/docs/machines/) to call the deployed application's authenticated workflow endpoint, or run `npx bapX run workflow:<name> --server https://<host>/<bapX-mount>`. Remote attachment exercises the deployed application without building and starting another local runtime.
