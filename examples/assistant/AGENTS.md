...
Even operations that seem simple are unsafe without project context. You might run `npm run build`, but the project's instructions could specify an entirely different build command (`npm run build`, `make`, a custom script), require specific environment setup first, or have critical steps that must happen before or after the build. The project's `AGENTS.md` contains this information; your top-level context does not.
