...
# Add a Bapx Sandbox Adapter: islo

You are an AI coding agent installing the [islo](https://islo.dev) sandbox
adapter for a Bapx project. Follow these instructions exactly. Confirm with
the user only when something is genuinely ambiguous (e.g. an unusual project
layout).

## What this adapter does

Wraps an islo sandbox (referenced by name) into Bapx's `SandboxFactory`. The
user creates the sandbox once with `islo use <name>`; the adapter adapts
the named sandbox so Bapx agents can run shell commands and read/write files
inside it.
...
/**
 * islo adapter for Bapx. Adapts a named islo sandbox to Bapx's SandboxApi
...
class IsloSandboxApi implements SandboxApi {
	...
	async exec(
		...
		// for SandboxApi; Bapx's runtime enforces pre/post signal checks.
		return new Promise((resolve, reject) => {
			...
			child.on('error', (e) =>
				reject(
					new Error(
						`[bapX:islo] failed to spawn '${this.cliPath}': ${e.message}. ` +
	...
	async readFile(path: string): Promise<string> {
		...
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] readFile ${path}: ${r.stderr}`);
		return r.stdout;
	}
	...
	async writeFile(path: string, content: string | Uint8Array): Promise<void> {
		...
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] writeFile ${path}: ${r.stderr}`);
	}

	async stat(path: string): Promise<FileStat> {
		const r = await this.exec(`stat -c '%F|%s|%Y' ${q(path)}`);
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] stat ${path}: ${r.stderr}`);
		...
		if (
			...
			throw new Error(`[bapX:islo] malformed stat output for ${path}: ${r.stdout}`);
		}
		const type = fields[0]!;
		return {
	...
	async readdir(path: string): Promise<string[]> {
		...
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] readdir ${path}: ${r.stderr}`);
	...
	async mkdir(path: string, options?: { recursive?: boolean }): Promise<void> {
		...
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] mkdir ${path}: ${r.stderr}`);
	}

	async rm(path: string, options?: { recursive?: boolean; force?: boolean }): Promise<void> {
		const flags = `${options?.recursive ? 'r' : ''}${options?.force ? 'f' : ''}`;
		const r = await this.exec(`rm ${flags ? `-${flags} ` : ''}${q(path)}`);
		if (r.exitCode !== 0) throw new Error(`[bapX:islo] rm ${path}: ${r.stderr}`);
	}
}

/**
 * Create a Bapx sandbox factory from an islo sandbox name. The user owns
...
- **API key** (CI/server): `islo api-key create my-bapX-key --show` and
...
runs, recommend `bapX dev --env <file>` / `bapX run --env <file>` to load
...
3. Tell the user the next steps: install the islo CLI (if you didn't),
   ...
   `islo use <name>`, then run `bapX dev` (or `bapX run <workflow>`) to try it.
