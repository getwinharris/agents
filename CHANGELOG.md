...
- Public docs now separate deployed, authentication-gated, partial, and planned bapX capabilities; remove false dashboard agent-creation, instant-hosting, team-role, connector, and live shared API/MCP claims; document MediaHub/FDE as a separate custom-quote service; and expose the existing CLI, runtime API, SDK, and reference pages through a Developers navigation section (#121).
...
- **Public developer contract boundary.** Supported CLI, build, API, SDK, runtime, configuration, and developer contracts remain published on `docs.bapx.in` for implementers and coding agents. Hosted product customers can use Platform, Agents, Admin-equivalent workspaces, API, MCP, and connectors without installing the CLI; only private host mechanics, secrets, incident procedures, unreleased operator playbooks, and exploitable operations stay repository-only.
