...
- **Public product UI and design-system consolidation.** The landing, products, and pricing pages now present bapX as an agent-team operating surface rather than a static feature grid: the hero shows a central bapX agent coordinating specialist sub-agents, the page adds benchmark comparison, work-loop, agent-timeline, OKF/code-window, capability-ledger, connector, and India/Razorpay pricing proof components, and the public nav is normalized to direct ecosystem links. The external design contract in `/root/bapx.in/DESIGN.md` now records the consolidated Claude, Cursor, Expo, ElevenLabs, Linear, MiniMax, and xAI lessons while preserving bapX's dark-canvas, white-pill, violet/orange brand system.
...
- **Ecosystem coverage.** The docs and catalog add Razorpay, bapX Host, bapXdb, OpenSandbox, and cloud coding CLIs as explicit planned/owned ecosystem surfaces so customer-facing navigation does not link to missing pages or undocumented capabilities.
