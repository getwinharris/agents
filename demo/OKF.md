...
- `type` — A short string identifying the kind of concept. Consumers
  ...
  `CLI Query`, `JSON Schema Collection`, `YAML Collection`, `API Endpoint`,
  `Workflow`, `Tool`, `Playbook`, `Reference`.
...
---
type: CLI Query
title: Local Orders Query
description: Reads completed customer orders from the local bapX query layer.
resource: cli:bapX db query orders --limit 5
tags: [local-db, orders, revenue]
timestamp: 2026-07-10T20:14:00Z
---

# Query

    bapX db query orders --limit 5

# Schema

The query result must match [orders.schema.json](/schemas/orders.schema.md).

| Field         | Type     | Description                              |
|---------------|----------|------------------------------------------|
| `order_id`    | string   | Globally unique order identifier.        |
| `customer_id` | string   | Foreign key into [customers](/collections/customers.md). |
| `total`       | number   | Order total in the project currency.     |
| `placed_at`   | datetime | When the customer submitted the order.   |

# Related Collections

- [orders.yaml](/collections/orders.md)
- [customers.yaml](/collections/customers.md)
```
...
## 2026-05-22
* **Update**: Added new local query reference for [Customer Metrics](/queries/customer-metrics.md).
* **Creation**: Established the [Collection Quality Playbook](/playbooks/collection-quality.md).
...
[1] [Local collection schema](/schemas/orders.schema.md)
...
`collections/orders.md`:

```markdown
---
type: YAML Collection
title: Orders Collection
description: Local YAML collection for completed customer orders.
resource: file:collections/orders.yaml
tags: [orders, local-db]
timestamp: 2026-07-10T20:14:00Z
---

The orders collection is queried through [Local Orders Query](/queries/orders.md)
and validated against [orders.schema.json](/schemas/orders.schema.md).
```
...
type: CLI Query
title: Orders
description: Query local order records through the bapX CLI.
resource: cli:bapX db query orders
tags: [orders, query]
timestamp: 2026-07-10T20:14:00Z
---
...
# Examples
    ...
    bapX db query orders --where status=completed --limit 25

Part of the [orders collection](/collections/orders.md).
```

---

## Appendix B — bapX user workspace profile

This appendix defines the bapX workspace profile for using OKF inside
`/root/bapx.in/users/`. It is an OKF profile: the files remain plain
markdown, git-friendly, and agent-readable.

### B.1 User repository structure

Every user workspace is a git repository. Businesses live inside the
user repository, and each business can own several projects:

```
/root/bapx.in/users/<username>/
├── .git/                         # User-owned history, diffs, and rollback.
├── OKF.md                        # Copy of the root OKF document.
├── index.md                      # User workspace entry point.
├── map.mmd                       # Mermaid map of the user workspace.
└── <business-slug>/
    ├── index.md                  # Business entry point.
    ├── map.mmd                   # Mermaid map of the business workspace.
    ├── DESIGN.md                 # Business design principles and brand identity.
    ├── brand.css                 # Business-level brand tokens.
    ├── logos/
    │   ├── index.md
    │   └── map.mmd
    └── projects/
        ...
        └── <project-name-slug>/
            ...
            ├── map.mmd           # Generated project map.
            ├── docs/
            │   ├── index.md      # Project documentation entry point.
            │   └── map.mmd
            └── ...
```

The username directory is the ownership boundary. A user can manage
many businesses under `users/<username>/<business-slug>/`, and each
business can manage many projects under `projects/<project-name-slug>/`
without creating parallel repositories outside
`/root/bapx.in/users/<username>/`.

### B.2 Required user workspace files

Each `users/<username>/` repository MUST include:

- `OKF.md`, copied exactly from `/root/bapx.in/OKF.md`.
- `index.md`, linking to `OKF.md`, `map.mmd`, and each business folder.
- `map.mmd`, describing the user workspace.
- One or more business folders at `users/<username>/<business-slug>/`.

Do not create a fake `users/demo` workspace. Canonical demo and example
sources remain in `/root/bapx.in/project-packages-git/agents/demo/`
and `/root/bapx.in/project-packages-git/agents/examples/`.

### B.3 Required business files

Each `users/<username>/<business-slug>/` business MUST include:

- `index.md`, linking to `DESIGN.md`, `brand.css`, `logos/`, `projects/`,
  and `map.mmd`.
- `map.mmd`, describing the business workspace.
- `DESIGN.md`, containing design principles and brand identity gathered
  during setup.
- `brand.css`, containing business-level brand variables.
- `logos/`, with `logos/index.md` and `logos/map.mmd`.
- `projects/`, with `projects/index.md` and `projects/map.mmd`.

### B.4 Required project files

Each `users/<username>/<business-slug>/projects/<project-name-slug>/` project MUST
include:

- `index.md`, linking to the project map, docs, and main source entry
  points.
- `map.mmd`, generated from the project code and docs. It must surface
  folders, functions, classes, tools, workflows, connections, and known
  gaps where the project exposes them.
- `docs/`, with at least `docs/index.md` for project-specific
  documentation and `docs/map.mmd` for the docs folder.
...
Projects MAY include additional folders such as `src/`, `assets/`,
`public/`, `content/`, `tests/`, or deployment-specific files.

### B.5 Markdown frontmatter

Every markdown concept document in a user, business, project, or project
docs folder SHOULD use YAML frontmatter. For project overview docs, use
this shape unless a local docs schema already requires more specific
fields:
...
Folder `index.md` files MAY be generated indexes, but they should still
be useful entry points for humans and agents. Every folder in the user,
business, and project structure MUST have both `index.md` and `map.mmd`.

### B.6 Map generation and validation

Use the bapX CLI as the source of truth for project maps:

```bash
bapX map --root /root/bapx.in/users/<username>/<business-slug> --check --profile business-workspace
bapX map --root /root/bapx.in/users/<username>/<business-slug>/projects/<project-name-slug>
bapX map --root /root/bapx.in/users/<username>/<business-slug>/projects/<project-name-slug> --check --profile user-project
```

`bapX map --check` validates that `map.mmd` exists and matches the
current project layout. The `user-project` profile also checks that
`index.md`, `docs/`, `docs/index.md`, and `docs/map.mmd` exist. If the
business or project structure changes, regenerate the affected maps and
commit the updated files in the user's git repository.
